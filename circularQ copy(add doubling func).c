 ...
 */
// 요구사항을 만족시킨 후, 기능이 조금 부족하다는 생각이 들어서 Full 상황에서의 더블링 기능도 dequeue를 재활용해서 간단하게 넣어봤습니다.
#include <stdio.h>
#include <stdlib.h>

int MAX_QUEUE_SIZE = 4; // 큐의 기본 크기는 4로 설정 (원형 큐이므로, 실질적으로 저장 가능한 데이터는 3개임. 필요시 더블링을 위해 전역 변수로 선언)
...
QueueType *queue_double(QueueType *cQ)
{
	...
	int i;
	char item;
	int re_max_queue_size = 2 * MAX_QUEUE_SIZE; // 새롭게 만들 re_CQ의 사이즈
	...
	// 기존 내용 옮기고, debug시 이상한 값 들어있는 것을 제거하기 위한 절차 수행
	re_cQ->queue[0] = '\0'; // front 부분

	for(i = 1; i < MAX_QUEUE_SIZE; i++)
	{
		deQueue(cQ, &item); // deQueue를 활용하여, 이전 큐의 front부터 rear까지 차례대로 추출하여 복사
		re_cQ->queue[i] = item;
	}

	for(i = re_cQ->rear+1; i < re_max_queue_size; i++) // debug시 이상한 값 들어있는 것을 제거하기 위한 절차 수행
	{
		re_cQ->queue[i] = '\0';
	}

	MAX_QUEUE_SIZE = re_max_queue_size; // 전역변수를 변경함
...
void deQueue(QueueType *cQ, element *item)
{
    if(!isEmpty(cQ))
	{
		*item = cQ->queue[(cQ->front+1)%MAX_QUEUE_SIZE];
		cQ->queue[(cQ->front+1)%MAX_QUEUE_SIZE] = '\0'; 	// debug시 값이 그대로 남는 것을 제거하기 위한 절차 추가 (그렇기 때문에 교수님 영상의 결과와는 조금의 차이가 있습니다.)
